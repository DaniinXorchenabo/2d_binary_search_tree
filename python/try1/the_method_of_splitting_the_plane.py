...
class MyPoint(object):
    ...
    def create_enter_point(cls, points_: list[MyPoint], color=(255, 0, 0), radius=10):
        agv_x, agv_y = sum([i.x for i in points_]) / len(points_), sum([i.y for i in points_]) / len(points_)
        x_sorted = sorted(points_, key=lambda i: i.x)
        y_sorted = sorted(points_, key=lambda i: i.y)
        len_ = len(points_)

        min_x_line, min_y_line, max_x_line, max_y_line = 0,0,0,0
        # for p in x_sorted:
        #     if p

        if len_ % 2 == 0:
            min_x, max_x = x_sorted[len_ // 2 - 1].x, x_sorted[len_ // 2].x
            min_y, max_y = y_sorted[len_ // 2 - 1].y, y_sorted[len_ // 2].y
            if min_x < agv_x < max_x:
                new_x = agv_x
            elif min_x >= agv_x:
                new_x = min_x + (max_x - min_x) * 0.01
            else:
                new_x = max_x - (max_x - min_x) * 0.01

            if min_y < agv_y < max_y:
                new_y = agv_y
            elif min_y >= agv_y:
                new_y = min_y + (max_y - min_y) * 0.01
            else:
                new_y = max_y - (max_y - min_y) * 0.01
    ...
    def find_point_in_tree(cls, screen_, find_point_: MyPoint, points_tree: list[Optional[MyPoint]], start_index: int,
        ...
        if len(points_tree) <= index or (new_start_point := points_tree[index]) is None:
            ...
            gen = cls.find_point_in_tree(screen_, find_point_, points_tree, index,
                                         deep=deep + 1,
                                         number_group=int(start_point.x < find_point_.x) + int(
                                             start_point.y < find_point_.y) * 2,
                                         # last_x_index=number_group % 2 + last_x_index * 2,
                                         # last_y_index=number_group // 2 + last_y_index * 2,
    ...
    def __repr__(self):
        return f'{self.int_x},{self.int_y}'  # , {self._color[0] == self._color[1] == self._color[2]}
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    ddd()
                if False and event.key == pygame.K_SPACE and can_create_point is True:  #
                    # print('K_SPACE')
            ...
            if False and event.type == pygame.MOUSEBUTTONDOWN and can_create_point is True:
                ...
                for deep in range(1, 5):
                    ...
                    print(*["|".join([str(tree_list[i + j]).center(7) for j in range(2 ** (deep))]) for i in
                            range(st, end, 2 ** (deep))], '---\n', sep='\n')

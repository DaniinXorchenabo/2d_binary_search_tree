...
from python.try1.try6.structures.queue_node import QueueNode


class PriorityQueue(AbstractTree[QueueNode]):

    def __repr__(self):
        m: list[list[QueueNode]] = []
        s: list[list[str]] = []
        arr: list[tuple[QueueNode, int, int]] = [(self.tree, 0, 4)]
        next_arr: list[tuple[QueueNode, int, int]] = []
        str_len = 1
        count = 0
        while bool(arr):
            count += 1
